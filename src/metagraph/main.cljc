(ns metagraph.main
  "Kotodama WASM entrypoint for the Metagraph clean-room actor (L4) — Clojure port.
  ...
  facts (`metagraph.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "metagraph")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"metagraph.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "metagraph-compat" :tier tier :entities entities} 200])
...
(defn start! [] :metagraph-compat/ready)
